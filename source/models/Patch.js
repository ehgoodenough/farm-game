...
export default class Patch {
    ...
    onClick() {
        if(model.inventory.selectedItem === undefined) {
            return
        }
        if(model.inventory.selectedItem.key === "SAPLINGS"
        && this.tree === undefined) {
            this.tree = {"stage": 1}
            console.log("Planted tree! Set to stage 1.")
        }
        if(model.inventory.selectedItem.key === "WATERING_CAN"
        && this.tree !== undefined
        && this.tree.stage < 5) {
            this.tree.stage += 1
            console.log("Watered tree! Grew by 1 stage.")
        }
        if(model.inventory.selectedItem.key === "AXE"
        && this.tree !== undefined
        && this.tree.stage === 5) {
            this.tree = undefined
            model.inventory.resources.wood += 5
